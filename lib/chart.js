      ...
      if (currentEvent) {
        ...
        var partialEvent = this.props.events.find(function (event) {
          var eventEnd = new _range_date2['default'](event.startDate).advance('days', event.duration),
              from = _this.props.range.from.value().setHours(0, 0, 0, 0),
              eventStart = new Date(event.startDate).setHours(0, 0, 0, 0);

          return eventEnd.toRef() === date && from.valueOf() > eventStart.valueOf() && event.resource === resource;
        });
    ...
    value: function renderCell(resource, date) {
      ...
      return _react2['default'].createElement(
        'div',
        {
          ...
          key: '' + resource + '' + date,
          style: Object.assign({ height: this.props.rowHeight }, _styles.cellWrapper) },
